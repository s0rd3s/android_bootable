...
int applysignature_buf(char *sigfile_buf, int sigfile_buf_size, const char *partition_name){
  ...
  if(0 == partition_info(partition_name, part_dev_path, &storage_type)){
    ...
    if(2 == storage_type){
      ...
      printf("erase_offset: %lu(erase_size: %d), partion_end_offset: %lu\n", erase_offset, erase_size, partion_end_offset);
      if (erase_offset != partion_end_offset) {
        ...
        if(rsize != partion_end_offset-erase_offset){
            printf("%s: fail to read %s, size:%lu, off:%lu, readback:%d\n", __func__, part_dev_path, partion_end_offset-erase_offset, partion_end_offset, rsize);
            result = -1;
            mtd_read_close(r_ctx);
            free(page_buf);
            goto done;
...
int applysignature(const char *filename, const char *partition_name){
  ...
  if (in_fd != -1) {
    ...
    if (rbuf == NULL) {
      printf("Fail to allocate %ju buffer\n", f_buf.st_size);
      close(in_fd);
      return -1;
    }
