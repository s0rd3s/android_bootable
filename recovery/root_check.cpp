...
unsigned int root_check_key=15;
...
img_info_t img_array_gpt[PART_MAX] = {
#ifdef MTK_ROOT_PRELOADER_CHECK
    {"/dev/preloader", "preloader",0},
#endif
    {"/dev/block/platform/mtk-msdc.0/by-name/lk", "lk.bin","lk1",1,1},
    {"/dev/block/platform/mtk-msdc.0/by-name/lk2", "lk.bin","lk2",1,0},
    {"/dev/block/platform/mtk-msdc.0/by-name/boot", "boot.img","boot",0,0},
    {"/dev/block/platform/mtk-msdc.0/by-name/recovery", "recovery.img","recovery",1,0},
    {"/dev/block/platform/mtk-msdc.0/by-name/logo", "logo.bin","logo",1,0},
#if defined(MTK_ATF_SUPPORT) || defined(TRUSTONIC_TEE_SUPPORT)
    {"/dev/block/platform/mtk-msdc.0/by-name/tee1", "trustzone.bin","tee1",1,1},
    {"/dev/block/platform/mtk-msdc.0/by-name/tee2", "trustzone.bin","tee2",1,0},
#endif
};
...
static int check_reall_file(char* path, unsigned int nCS, char* nMd5)
{
    int found=0;
    int ret=0;
    FILE *fp_info;
    ...
    if(fp_info)
        {
                while (fgets(buf, sizeof(buf), fp_info)) {
                    ...
                    if (sscanf(buf,"%s    %s    %s    %s",p_name,p_img_size,p_crc,p_md5) == 4) {
                            char *p_pname=decrypt_str(p_name,root_check_key);
                            char *p_pcrc=decrypt_str(p_crc,root_check_key);
                            char *p_pmd5=decrypt_str(p_md5,root_check_key);
                            char *p_pimgsize=decrypt_str(p_img_size,root_check_key);
                            unsigned long crc;
...
static int set_checksum_result_expected(img_info_t *image_info, const char *name, unsigned int res_crc, unsigned char *res_md5, int img_size)
{
   int i = 0;

   for(i=0;i<PART_MAX;i++,image_info++)
   {
     if(strstr(name,image_info->img_printname)!=NULL)
     {
        image_info->expected_crc32 = res_crc;
        image_info->image_size = img_size;
        memcpy(image_info->expected_md5, res_md5, MD5_LENGTH);

        if(image_info->duplicate)
        {continue;}
...
static int set_checksum_result_computed(img_info_t *image_info, const char *name, unsigned int res_crc, unsigned char *res_md5, int img_size)
{
   int i = 0;

   for(i=0;i<PART_MAX;i++,image_info++)
   {
     if(strstr(name,image_info->part_name)!=NULL)
     {
        image_info->computed_crc32 = res_crc;
        image_info->image_size = img_size;
        memcpy(image_info->computed_md5, res_md5, MD5_LENGTH);
    ...
    while ((dp = readdir(d_fd)) != NULL) {
        ...
        if (dp->d_type == DT_DIR){
           ...
           strcat(newdir, dp->d_name);
               if(0 == file_crc_check(newdir, &nCS, nMd5)){
                  if(!is_support_gpt_c())
                     ret = set_checksum_result_computed(img_array, newdir, nCS, nMd5, img_size);
                  else
                     ret = set_checksum_result_computed(img_array_gpt, newdir, nCS, nMd5, img_size);

                  if(ret < 0)
                  {
                    ui->Print("Error: image_crc_check Compute partition fail\n");
                    return false;
                  }
               }else{
...
static int image_copy(img_info_t *image_info, const char* path,int loop,const char* tem_name)
{
    unsigned int nCS=0;
    char *temp_file;
    char buf[1024];
    ...
    if (st.st_uid != 0) {
        LOGE("%s owned by %u; not root\n", IMAGE_LOAD_PATH, (unsigned int) st.st_uid);
      return -1;
    }
    ...
    while ((read = fread(buffer, 1, 1024, fin)) > 0) {
      ...
      else
      {
          ...
          read_end  = ((read+loop-sum) > 0)  ? read+loop-sum : 0;
...
static int get_image_info()
{
    ...
    unsigned char expected_md5[MD5_LENGTH*2];
    int ret = -1;
    ...
    if(fp_info)
        {
                while (fgets(buf, sizeof(buf), fp_info)) {
                    ...
                    if (sscanf(buf, "%s %d %u %s", p_name, &p_size, &expected_crc, expected_md5) == 4) {
                        hextoi_md5(expected_md5);
                        if(!is_support_gpt_c())
                          ret = set_checksum_result_expected(img_array, p_name, expected_crc, expected_md5, p_size);
                        else
                          ret = set_checksum_result_expected(img_array_gpt, p_name, expected_crc, expected_md5, p_size);

                        if(ret < 0)
                        {
                          ui->Print("Error: get_image_info Compute partition fail\n");
                          return -1;
                        }
                    }
                }
...
int root_check(){
    ...
    if(!is_support_gpt_c())
      image_compare = img_array;
    else
      image_compare = img_array_gpt;
    ...
    {
         if(0 == image_copy(&image_compare[i],image_compare[i].img_devname,image_compare[i].image_size,image_compare[i].part_name))
         {
             printf("copy %s done\n", image_compare[i].part_name);
         }
         else
         {
             printf("copy %s error\n", image_compare[i].part_name);
    ...
    {
#ifdef MTK_ROOT_NORMAL_CHECK
        if((image_compare[i].expected_crc32==image_compare[i].computed_crc32)&&(image_compare[i].computed_crc32 != 0))
              printf("\n%s NORMAL check Pass", image_compare[i].part_name);
        else
        {
            if(!image_compare[i].ignore)
            {
                checkResult=false;
                ui->Print("Error:%s NORMAL check Fail\n",image_compare[i].part_name);
                printf("except check sum is %u,compute checksum is %u\n",image_compare[i].expected_crc32,image_compare[i].computed_crc32);
            }
        }
#endif

#ifdef MTK_ROOT_ADVANCE_CHECK
        if(memcmp(image_compare[i].expected_md5, image_compare[i].computed_md5, MD5_LENGTH)==0)
        {
           printf("\n%s ADVANCE check Pass", image_compare[i].part_name);
        }
        else
        {
            if(!image_compare[i].ignore)
...
#if 1
                char *nMd5 = (char*)image_compare[i].expected_md5;
                char *p_md = (char*)image_compare[i].computed_md5;
                int i;
                printf("e:");
                for(i=0;i<16;i++)
                {
