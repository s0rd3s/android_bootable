...
Value* MountFn(const char* name, State* state, int argc, Expr* argv[]) {
	...
	if (mt_MountFn_cache_merge_init(&mount_point, &result))  /* CACHE_MERGE_SUPPORT */
		goto done;
	...
	int ret=mt_MountFn(&mount_point, &result, &fs_type, &location,
	...
	mt_MountFn_cache_merge_final(&mount_point, &result);  /* CACHE_MERGE_SUPPORT */
    free(fs_type);
    free(partition_type);
    free(location);
...
Value* UnmountFn(const char* name, State* state, int argc, Expr* argv[]) {
	...
	if (mt_UnmountFn_chache_merege(&mount_point, &result))
		goto done;
    scan_mounted_volumes();
    const MountedVolume* vol = find_mounted_volume_by_mount_point(mount_point);
    if (vol == NULL) {
	...
	if (mt_UnmountFn_ubifs(mount_point))
        result = strdup("");
...
#else
	if (mt_FormatFn(&mount_point, &result, &fs_type, &location, &partition_type, name, fs_size))
		goto done;
#endif
...
Value* PackageExtractFileFn(const char* name, State* state,
    ...
    if (argc == 2) {
		...
		Value* retval_ubifs=NULL;
		if (mt_PackageExtractFileFn_ubifs(name, dest_path, zip_path, za, entry, &retval_ubifs, &success))
			goto done2;
		if (retval_ubifs)
			return retval_ubifs;
        ...
        struct perm_parsed_args parsed)
{
    ...
    if (parsed.has_capabilities && S_ISREG(statptr->st_mode)) {
        if (parsed.capabilities == 0) {
            if ((removexattr(filename, XATTR_NAME_CAPS) == -1) && (errno != ENODATA) && (errno != EOPNOTSUPP)) {
            ...
            if (setxattr(filename, XATTR_NAME_CAPS, &cap_data, sizeof(cap_data), 0) < 0 && (errno != EOPNOTSUPP)) {
...
Value* WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (mt_WriteRawImageFn(state, &partition, &result, &partition_value, &contents))
        goto done;
...
Value* RebootNowFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    sleep(5);
    free(property);
#if 1 //wschen 2014-10-01 backward workaround for JB
...
Value* SetStageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    fclose(f);
#else
    Value *ret = mt_SetStageFn(name, state, &filename, &stagestr);
    if (ret) return ret;
#endif
...
Value* GetStageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    fclose(f);
#else
	Value *ret = mt_GetStageFn(name, state, &filename, buffer);
	if (ret) return ret;
#endif
