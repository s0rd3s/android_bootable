...
int userdata_backup(uint64_t backup_size)
{
    ...
    if (fp) {
        fgets(backup_path, sizeof(backup_path), fp);
        fclose(fp);
        unlink(backup_path);
    ...
    /*
       L always use raw copy due to data encrypt and selinux
       When use file based and selinux is closed , it use tar main to backup
    */
    ...
    if ((phone_encrypted == 0) && (!force_raw_backup)) {
        ...
        if ((val = setjmp_env()) != 0) {
            ...
            unlink("/sdcard/.data_backup_name");
            sync();
            child_pid_num = get_child_pid();
            if (child_pid_num != 0) {
                kill((pid_t)child_pid_num, SIGTERM);
                wait(NULL);
            }
        ...
        tar_main(0, backup_path, (char *)"/data");
        sync();
        printf("tmp_backup_path= %s\n", tmp_backup_path);
        fp = fopen(backup_path, "r+");
        if (fp) {
            uint8_t *hash_value = hash_file(backup_path);
            if (hash_value == NULL) {
                ui->Print("Error: fail to generate checksum\n");
                fclose(fp);
                chdir("/");
                ensure_path_unmounted("/sdcard");
                ensure_path_unmounted("/data");
            ...
            my_header.file_count = get_total_files();
            my_header.encrypt = phone_encrypted;
            my_header.size = data_used;
            my_header.raw_file_count= 1;
            memcpy(my_header.checksum, hash_value, COMPARE_NUM);
            free(hash_value);
            set_encrypted_custom_header(plan, sizeof(plan), &my_header);
            fwrite(plan, sizeof(plan), 1, fp);
            fclose(fp);
        } else {
            ui->Print("Error: Open backup file fail.\n");
            chdir("/");
        ...
        //phone encrypted, dump raw data
        char blk[128];
        if (is_support_gpt()) {
            snprintf(blk, sizeof(blk), "%s", DATA_PART);
        } else {
            get_partition_blk_dev("usrdata", blk);
        }
        ...
        if (do_raw_backup_restore(blk, backup_path, data_used, ACTION_BACKUP) < 0) {
...
//    char key_data[16];
    ...
    do {
        ...
        if (fp) {
            fread(my_buf, sizeof(my_buf), 1, fp);
            fclose(fp);
        } else {
        ...
        if (hash_value != NULL) {
            if (memcmp(hash_value, my_header.checksum, COMPARE_NUM) != 0) {
                ui->Print("Error: Checksum compare fail\n");
                free(hash_value);
                return INSTALL_CORRUPT;
            }
            free(hash_value);
    ...
    /*
       L always use raw copy due to data encrypt and selinux
       When use file based and selinux is closed , it use tar main to restore
    */
    if (my_header.encrypt == 0 && (!force_raw_backup) ) {

        if (ensure_path_mounted("/data") != 0) {
            ui->Print("Error: Mount user data fail\n");
            return INSTALL_ERROR;
        }
        ...
        if (do_raw_backup_restore(filename, blk, my_header.size, ACTION_RESTORE) < 0) {
...
int do_resize2fs(const char *dev_name, uint64_t new_part_size)
{
    ...
    if (WIFEXITED(status)) {
        if (WEXITSTATUS(status) > 1) {
            LOGE("Error for %s(Status %d)\n", command, WEXITSTATUS(status));
            ui->Print("Error for %s(Status %d)\n", command, WEXITSTATUS(status));
            if (system_mount_change)
...
int check_part_size(ZipArchive *zip, int path_is_data, const char *zip_path, MemMapping* pMap,bool needs_mount)
{
    ...
    if (part_num != mtd_cnt) {
        ...
        if (match_cnt) {
            ...
            if (path_is_data) {
                printf("Updating from /data without it's layout change, skip SD card check.\n");
            } else {
                if (!phone_encrypted && (sd_free <= data_used)) {
                    ui->Print("SD card did not have enough free space.\n");
                    ui->Print("SD free %llu MB, data used %llu MB.\n", sd_free >> 20, data_used >> 20);
                    goto fail_out;
                }
            ...
            if (!phone_encrypted) {
                ...
                if (usrdata_changed) {
                    ...
                    if (get_emmc_phone() && (isResizeFail == 0) && (new_data_size < data_used)) {
                        if (recovery_is_permissive() == false) { // enforcing mode, resize before raw backup
                            clear_force_upgrade();
                            if (do_resize2fs(DATA_PART, new_data_size) < 0) {
                                ui->Print("Error: Resize partition fail\n");
                                isResizeFail = 1;
