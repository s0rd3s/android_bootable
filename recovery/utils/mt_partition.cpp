            ...
            if(parse_partition_info_by_index(i, &part_info))  {
                ...
                if (part_mtd_ptr != NULL) {
                    ...
                    if (!strcmp(part_info.part_name, "system")) {
                        if (!strcmp(part_mtd[num - 1]->name, "expdb")) {
...
char* mt_GetPartFullName(char *partition_name)
{
    char *part_fullname = NULL;
    char part_name[128];
    bool is_gpt = support_gpt();

    if (phone_type == EMMC_TYPE) {
        if (!strcmp(partition_name, "preloader")) {
            part_fullname = strdup(PRELOADER_PART);
            return part_fullname;
        } else if (!strcmp(partition_name, "preloader2")) {
            part_fullname = strdup(PRELOADER2_PART);
            return part_fullname;
        }
    } else {
        if (!strncmp(partition_name, "preloader", strlen("preloader"))) {
            snprintf(part_name, sizeof(part_name), "/dev/%s", partition_name);
            part_fullname = strdup(part_name);
            return part_fullname;
        }
    }

    if (is_gpt) {
        if (!strcmp(partition_name, "bootimg")) {
            part_fullname = strdup(BOOT_PART);
        } else if (!strcmp(partition_name, "uboot")) {
            part_fullname = strdup(LK_PART);
        } else {
            snprintf(part_name, sizeof(part_name), "%s/%s", MT_GPT_PART, partition_name);
            part_fullname = strdup(part_name);
        }
    } else {
        FILE *fp = fopen("/proc/dumchar_info", "r");
            ...
            while (fgets(buf, sizeof(buf), fp)) {
                if (sscanf(buf, "%s %s %s %d %s", p_name, p_size, p_addr, &p_type, p_actname) == 5) {
                    ...
                    if (strlen(p_name) && (!strcmp(p_name, "boot") || !strcmp(p_name, "bootimg"))) {
                        ...
                        if ((p_type == 2) && !strcmp(partition_name, "boot")) {
                            snprintf(part_name, sizeof(part_name), "/dev/bootimg");
                            part_fullname = strdup(part_name);
                            break;
                       } else if ((p_type == 1) && !strcmp(partition_name, "boot")) {
                            //phone is NAND and recovery.fstab is NAND
                            break;
                        } else if ((p_type == 1) && !strcmp(partition_name, "bootimg")) {
                            //phone is NAND but recovery.fstab is eMMC
                            snprintf(part_name, sizeof(part_name), "/dev/boot");
                            part_fullname = strdup(part_name);
                            break;
                        }
                    } else {
                        if (strlen(p_name) && !strcmp(partition_name, p_name)) {
                            if (p_type == 2) {
