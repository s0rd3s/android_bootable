...
main(int argc, char **argv) {
    ...
    if (update_package) {
        ...
        if (strncmp(update_package, "CACHE:", 6) == 0) {
          ...
          update_package = mt_main_init_sys_fw_upgrade(update_package);
    }
    printf("\n");
...
#else
        mt_main_update_package(status, update_package, wipe_cache);
    ...
    #else
        mt_main_wipe_data(status, device, wipe_cache);
    #endif
    } else if (wipe_cache) {
    #if 0
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
    ...
    #else
        mt_main_wipe_cache(status);
    #endif
    } else if (mt_main_fota(status)) {
    ...
    mt_main_do_sys_fw_upgrade(update_package);
