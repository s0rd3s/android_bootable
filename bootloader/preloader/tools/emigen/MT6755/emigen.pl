...
## option for early porting, only for build pass
## when this option is set 0, the emigen will generate a dummy custom_emi.c and custom_emi.h
my $enable_EMIGEN=1;
use File::Basename;
...
#=head		  	
			    if ($DEV_TYPE2[0] eq "02")
			    {
				    ...
				    if ( $DEV_TYPE1[$iter_dis_dram] eq "00" && $LPDDR2_MODE_REG5[$iter] eq $LPDDR2_MODE_REG5[$iter_dis_dram])
				    {
				        if ( $DRAM_RANK0_SIZE[$iter] eq $DRAM_RANK0_SIZE[$iter_dis_dram] && $DRAM_RANK1_SIZE[$iter] eq $DRAM_RANK1_SIZE[$iter_dis_dram] )
				        {
                                            print "[Error] Both MODE_REG5(DRAM vendor_ID) and RANK0/RANK1 size should not be the same in the Combo list\n" ;

					    die "[Error] MODE_REG5(DRAM vendor_ID) and RANK0/RANK1 size should not be the same in the Combo list\n" ;
				        }
				    }
			    }
			    elsif ($DEV_TYPE2[0] eq "03")
			    {
				    ...
				    # check the MODE_REG5(DRAM vendor_ID) are unique
				    # if have same MODE5 in the list, send the build error
				    if ( $DEV_TYPE1[$iter_dis_dram] eq "00" && $LPDDR3_MODE_REG5[$iter] eq $LPDDR3_MODE_REG5[$iter_dis_dram])
				    {
				        if ( $DRAM_RANK0_SIZE[$iter] eq $DRAM_RANK0_SIZE[$iter_dis_dram] && $DRAM_RANK1_SIZE[$iter] eq $DRAM_RANK1_SIZE[$iter_dis_dram] )
				        {
                                            print "[Error] Both MODE_REG5(DRAM vendor_ID) and RANK0/RANK1 size should not be the same in the Combo list\n" ;
...
#=cut
    ...
    unless($ENV{PROJECT} eq "mt6755_evb"){
...
sub DeviceListParser_LPSDRAM
{
    ...
    while ($iter<$num_part_found)
    {
        ...
        $DRAMC_RKCFG_VAL[$id]                = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
        $DRAMC_TEST2_4_VAL[$id]              = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;                
...
# LPDDR3    
	...
	#rank size check END
        ...
        elsif ($DEV_TYPE[$id] eq "MCP(eMMC+DDR1)")
        {
            $DEV_TYPE1[$id] = "02" ;
            $DEV_TYPE2[$id] = "01" ;
            die "[Error][EMIgen] no support EMMC+DDR1.";    
        }
	...
	elsif ($DEV_TYPE[$id] eq "MCP(eMMC+PCDDR3)")
        {
            ...
            $DEV_TYPE2[$id] = "04" ;
	    $MCP_PCDDR3 = $MCP_PCDDR3 + 1 ;
	    die "[Error][EMIgen] no support eMMC+PCDDR3.";
        }
        else
        {
	...
	$nand_emmc_id_len = $ID_Length[$id];
        ...
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $DRAMC_RKCFG_VAL[$id] . ",\t\t/* DRAMC_RKCFG_VAL*/\n\t\t" ;
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $DRAMC_TEST2_4_VAL[$id] . ",\t\t/* DRAMC_TEST2_4_VAL*/\n\t\t" ;                
...
sub write_tag()
{
    ...
    print FILE pack("a24", "MTK_BLOADER_INFO_v22");
...
sub write_tag_one_element_default_LPDDR3()
{
    ...
    print FILE pack ("L", hex (lc("0x00025052"))) ;         # EMI_CONA_VAL
    $fs = $fs + 4 ;
    ...
    print FILE pack ("L", hex (lc("0x66CB4619"))) ;      # DRAMC_ACTIM_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x11000000"))) ;  # DRAMC_GDDR3CTL1_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x000484C3"))) ;      # DRAMC_CONF1_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x000053B1"))) ;    # DRAMC_DDR2CTL_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0xBFC90401"))) ;    # DRAMC_TEST2_3_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x00806E60"))) ;      # DRAMC_CONF2_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0xD1643642"))) ;    # DRAMC_PD_CTRL_VAL
    $fs = $fs + 4 ;
    ...
    print FILE pack ("L", hex (lc("0x91001E59"))) ;           # $DRAMC_ACTIM1_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x17000000"))) ;        # DRAMC_MISCTL0_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x000004F1"))) ;           # $DRAMC_ACTIM05T_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x002145C1"))) ;        # DRAMC_RKCFG_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x2701110D"))) ;        # DRAMC_TEST2_4_VAL
    $fs = $fs + 4 ;
    ...
    {
        #ddr2
        print FILE pack ("L", hex (lc("0x00E30001"))) ;        # DDR2_MODE_REG1
        $fs = $fs + 4 ;
...
sub write_tag_one_element()
{
    ...
    print FILE pack ("L", hex (lc($DRAMC_RKCFG_VAL[$id]))) ;        # DRAMC_RKCFG_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc($DRAMC_TEST2_4_VAL[$id]))) ;        # DRAMC_TEST2_4_VAL
    $fs = $fs + 4 ;
