...
struct cpu_info {
	const char *name;
	struct cpu_opp_info cpu_opp_info_LL[NR_OPPS];
	struct cpu_opp_info cpu_opp_info_L[NR_OPPS];
	int vmin_hi;
	int vmin_lo;
};

struct mt_cpu_dvfs_ops {
        unsigned int (*get_cur_freq_l)(void);
        unsigned int (*get_cur_freq_ll)(void);
