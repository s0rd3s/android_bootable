...
/* STATIC VARIABLES                                                           */
/*============================================================================*/
static bl_param_t *p_bldr_param = NULL;
...
//    disable_FBB_SW();
    ...
    /* note that if you use cmm file, the parameter is empty. */
    efuse_get_load_addr(p_bldr_param->bl_loadinfo[0].bl_load_addr);
#endif
...
int bldr_load_tee_part(char *name, blkdev_t *bdev, u32 *addr, u32 offset, u32 *size)
{
    ...
    /* size includes image padding and cert chain size */
...
if (1 == aarch64_slt_done())
{
    addr = CFG_MD3_ROM_MEMADDR;
    bldr_load_part("MD3_ROM", bootdev, &addr, &size);
}
#endif
...
if (1 == aarch64_slt_done())
{
    ...
    *jump_addr = addr;
    ...
    if(!(addr >= CFG_UBOOT_MEMADDR && addr+size <= CFG_UBOOT_MEMADDR+0x900000))
        print("Warnning: LK Out of boundary.\n");
...
void main(u32 *arg)
{
    ...
    /* sec_boot_check removed, image will be verified in loading process */
