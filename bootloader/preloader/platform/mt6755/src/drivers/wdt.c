...
static void mtk_wdt_reset(char mode)
{
    ...
    print("mtk_wdt_reset WDT MODE=%x\n",DRV_Reg32(MTK_WDT_MODE));
...
void mtk_wdt_init(void)
{
    ...
    mtk_wdt_restart();
#endif
    wdt_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);
    wdt_ctrl |= MTK_RG_MCU_LATH_EN;
    wdt_ctrl |= MTK_DEBUG_CTL_KEY;
    WRITE_REG(wdt_ctrl, MTK_WDT_DEBUG_CTL);

    mtk_wdt_reset_deglitch_enable();
    
    // set mcu_lath_en/spm_latch_en requir by RGU DE
    wdt_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);
    wdt_ctrl |= MTK_DEBUG_CTL_KEY;
    wdt_ctrl &= ~(MTK_RG_MCU_LATH_LEGACY_EN | MTK_RG_SPM_LATH_LEGACY_EN);
    WRITE_REG(wdt_ctrl, MTK_WDT_DEBUG_CTL);
    
    wdt_ctrl = READ_REG(MTK_WDT_LATCH_CTL);
    wdt_ctrl |= MTK_LATCH_CTL_KEY;
    wdt_ctrl |= MTK_RG_LATH_EN;
    wdt_ctrl &= ~(MTK_RG_MCU_LATH_SEL | MTK_RG_SPM_LATH_SEL);
    wdt_ctrl |= (MTK_RG_MCU_LATH_EN | MTK_RG_SPM_LATH_EN | MTK_RG_DRAMC_LATH_EN);
    wdt_ctrl |= (MTK_RG_DRAMC_RD_TEST_EN | MTK_RG_DRAMC_RDWT_TEST_EN);
    WRITE_REG(wdt_ctrl, MTK_WDT_LATCH_CTL);

    print("RGU %s:MTK_WDT_DEBUG_CTL(%x), MTK_WDT_LATCH_CTL(%x)\n", __func__,
    		READ_REG(MTK_WDT_DEBUG_CTL), READ_REG(MTK_WDT_LATCH_CTL));
}
...
void rgu_release_rg_dramc_conf_iso(void)
{
  ...
  print("RGU %s:MTK_WDT_DEBUG_CTL(%x)\n", __func__,wdt_dbg_ctrl);
    ...
    wdt_dbg_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);//20150729: LewisLi Add for reserse mode
    while(wdt_dbg_ctrl & MTK_RG_CONF_ISO) 
    {
        print("!!!! RGU %s:MTK_WDT_DEBUG_CTL(%x) DDR refresh not realease\n", __func__, wdt_dbg_ctrl);   
        wdt_dbg_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);
    }
    if(wdt_dbg_ctrl & MTK_DDR_RESERVE_RTA)//0x10000
    {
        print("RGU %s:MTK_WDT_DEBUG_CTL(%x) DDR RESERVE Before RST Success\n", __func__, wdt_dbg_ctrl);   
    }
    else
    {
        print("RGU %s:MTK_WDT_DEBUG_CTL(%x) DDR RESERVE Before RST Fail\n", __func__, wdt_dbg_ctrl);   
    }
}
...
void rgu_release_rg_dramc_sref(void)
{
  ...
  print("RGU %s:MTK_WDT_DEBUG_CTL(%x)\n", __func__,wdt_dbg_ctrl);
    ...
    wdt_dbg_ctrl = READ_REG(MTK_WDT_DEBUG_CTL);
    wdt_dbg_ctrl &= (~MTK_RG_DRAMC_SREF);//0x100
    wdt_dbg_ctrl |= MTK_DEBUG_CTL_KEY;
    WRITE_REG(wdt_dbg_ctrl, MTK_WDT_DEBUG_CTL);
    print("RGU %s:MTK_WDT_DEBUG_CTL(%x)\n", __func__,wdt_dbg_ctrl);
