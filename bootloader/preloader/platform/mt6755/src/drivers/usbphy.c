...
int phy_change_pipe_phase_a60810(struct u3phy_info *info, int phy_drv, int pipe_phase){
	...
	print("phy_change_pipe_phase_a60810: %d", pipe_phase);
...
void mt_usb_phy_init(void)
{
	...
	USB_CLRMASK(U3D_USB30_PHYA_REG2, RG_SSUSB_VUSB10_ON);
	usb_writel(0x20000000, 0x1129030c);
	USB_SETMASK(0x11290c64, 0x1<<16);  /* RG_SSUSB_RX_CMPWD_U3 */
	USB_SETMASK(0x11290c64, 0x1<<23);  /* RG_SSUSB_RX_CMPWD_PE1H */
#endif
}
void mt_usb_phy_poweron(void)
{
	...
	USB_SETMASK(U3D_USBPHYACR5, A60810_RG_USB20_HS_100U_U3_EN);
	/* 11, OTG enable */
...
void mt_usb_phy_savecurrent(void)
{
	...
	USB_CLRMASK(U3D_USBPHYACR5, A60810_RG_USB20_HS_100U_U3_EN);
	/* 18, wait 800us */
	mdelay(1);
	/* 19 */
...
{	
	...
	USB11PHY_SET8(0x6C, 0x11);
	USB11PHY_CLR8(0x6D, 0x3C);
	USB11PHY_SET8(0x6D, 0x01);
