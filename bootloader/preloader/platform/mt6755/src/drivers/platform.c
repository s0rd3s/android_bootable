...
void platform_vusb_on(void)
{
	...
	ret = pmic_config_interface( (kal_uint32)(PMIC_LDO_VUSB33_EN_ADDR),
			(kal_uint32)(1),
	...
	if (ret==0){
		print("[platform_vusb_on] VA10 select to 1.0V\n");
	}
	else{
		print("[platform_vusb_on] Failed to select VA10 voltage!\n");
	}
#endif
#endif
    return;
...
static void platform_emacro_enable_dcm()
{
    U32 emacro_dcm = 0x10206600;

   /*Turn on Emacro DCM*/
    *((P_U32)emacro_dcm) |= (1<<0);
}
...
static boot_reason_t platform_boot_status(void)
{
	...
	if (rtc_2sec_reboot_check()) {
		print("%s PMIC Full Reset by 2sec reboot check!\n", MOD);
...
void umc_init(void)
{
    ...
    print("umc check: 0x10206540 = 0x%x\n", DRV_Reg32(0x10206540));

    if (DRV_Reg32(0x10206540) & 0x40000000) // bit 30 = umc
    {
        print("umc set: 0x1000C600 = 0x%x\n", DRV_Reg32(0x1000C600));

        temp = DRV_Reg32(0x1000C600) & ~(0x0000FF00);
        DRV_WriteReg32(0x1000C600, temp | (0x72 << 8)); // 0x1000C600[15:8] = 0x72
...
//#endif
    ...
    /* Enable emacro dcm */
    platform_emacro_enable_dcm();

#if !CFG_FPGA_PLATFORM
    BOOTING_TIME_PROFILING_LOG("PMIC1");
    pmic_ret = pmic_init();
    BOOTING_TIME_PROFILING_LOG("PMIC2");
...
#define _B(n)   ((u64)((u8 *)&x)[n])
static inline u32 fdt32_to_cpu(u32 x)
{
        return (_B(0) << 24) | (_B(1) << 16) | (_B(2) << 8) | _B(3);
}

static inline u64 fdt64_to_cpu(u64 x)
{
        return (_B(0) << 56) | (_B(1) << 48) | (_B(2) << 40) | (_B(3) << 32)
                | (_B(4) << 24) | (_B(5) << 16) | (_B(6) << 8) | _B(7);
}

static void bldr_load_dtb(void)
{
    blkdev_t *bootdev;
    u32 scratch_addr = 0;
    u32 size = 0;
    ...
    //scratch_addr = CFG_DRAM_ADDR + 0x05000000; //it needs to be aligned with LK/fastboot scratch address
    scratch_addr = CFG_DRAM_ADDR + 0x06900000; //it needs to be aligned with LK/fastboot scratch address
    size = 0x1500000; //boot image size limit need to be check, current: 21MB
    ...
    print("%s bldr load bootloader img part addr=0x%x, size=0x%x, kernel_boot_opt=%d\n", MOD,  addr, size, g_kernel_boot_opt);
    bldr_load_bootimg_header("boot", bootdev, &scratch_addr, 0, &size);
    //g_kernel_boot_opt possible options:
    //BOOT_OPT_32S1 -> armv7
    //BOOT_OPT_32N2 -> armv8
    //BOOT_OPT_64N2 -> armv8 
    if (BOOT_OPT_32S1 == g_kernel_boot_opt ||
	BOOT_OPT_32N2 == g_kernel_boot_opt){
	// if booting kernel is 32 bit, dtb loader is here
	print("%s Kernel is 32 bit\n", MOD);
	...
	zimage_addr = scratch_addr+0x800;//bootimg header size is 0x800, the kernel img text is next to the header
        zimage_size = *(unsigned int *)((unsigned int)zimage_addr+0x2c) - *(unsigned int *)((unsigned int)zimage_addr+0x28);
        print("%s zimage_addr:0x%x, zimage_size:0x%x\n",MOD,zimage_addr, zimage_size);
        dtb_addr = (unsigned int)zimage_addr + zimage_size;
	dtb_size = fdt32_to_cpu(*(unsigned int *)(dtb_addr+0x4));
    } else if (BOOT_OPT_64N2 == g_kernel_boot_opt){
	// if booting kernel is 64 bit, dtb loader is here
	print("%s Kernel is 64 bit\n", MOD);
	//struct boot_img_hdr
	//{
	...
	//};
	zimage_addr = scratch_addr+0x800;//bootimg header size is 0x800, the kernel img text is next to the header
        zimage_size = *(unsigned int *)((unsigned int)scratch_addr+0x8); //zimage_size:(boot_hdr->kernel_size);
        zimage_end_addr = (unsigned int)(zimage_addr + zimage_size);
        print("%s zimage_addr=0x%x, zimage_size:0x%x, zimage_end_addr:0x%x\n",MOD, zimage_addr, zimage_size, zimage_end_addr);
	for(dtb_size = 0,addr = zimage_end_addr; dtb_size < zimage_size; dtb_size++, addr--)
        {
            ...
            {
                dtb_addr = addr;
                print("%s get dtb_addr=0x%x, dtb_size=0x%x\n",MOD, dtb_addr, dtb_size);
	...
	print("%s kernel boot opt:%d not support\n", MOD,g_kernel_boot_opt);
    }
    dtb_kernel_addr = *(unsigned int *)((unsigned int)scratch_addr+0x20); //dtb_kernel_addr:(boot_hdr->tags_addr);
    g_dtb_addr = dtb_kernel_addr;
    g_dtb_size = dtb_size;
    print("%s Copy DTB from 0x%x to 0x%x, dtb size:0x%x\n",MOD, dtb_addr, dtb_kernel_addr, dtb_size);
...
#if CFG_DTB_EARLY_LOADER_SUPPORT
    //load device tree blob to memory
    bldr_load_dtb();
#endif
