...
extern DRAM_STATUS_T Dram_Reset(DRAMC_CTX_T *p);
...
void Restore_Specific_Regs(void)
{
    ucDram_Register_Write(0x80, u4Value_Reg80);
    ucDram_Register_Write(0x63C, u4Value_Reg63C);
    ucDram_Register_Write(0x698, u4Value_Reg698);
    return;
}
...
void After_Exit_DDR_Reserved_Mode_setting(void)
{
    ...
    //Force to 3PLL mode, 12th and 14th bit set to 0 {[12] spm2dramc_shuffle_switch, [14] spm2dpy_1pll_switch}
    Reg_Sync_Writel((SPM_POWER_ON_VAL0), Reg_Readl((SPM_POWER_ON_VAL0)) & ~((0x1 << 12) | (0x1 << 14)));    
...
DDR_CALI_START: 
  ...
  p->channel = CHANNEL_A;
   ...
   if (p->dram_type == TYPE_LPDDR3)
    {  
        S8 iFirstClkPass[CATRAINING_NUM], iLastClkPass[CATRAINING_NUM], iFirstCAPass[CATRAINING_NUM], iLastCAPass[CATRAINING_NUM];
        S8 iFirstClkPass2[CATRAINING_NUM], iLastClkPass2[CATRAINING_NUM], iFirstCAPass2[CATRAINING_NUM], iLastCAPass2[CATRAINING_NUM]; 
        S8 iCAShift[CATRAINING_NUM];
        S8 iMaxCenter;

#ifdef CA_TRAINING_NEW_FLOW
        S8 iFirstClkPass_1PLL[CATRAINING_NUM], iLastClkPass_1PLL[CATRAINING_NUM], iFirstCAPass_1PLL[CATRAINING_NUM], iLastCAPass_1PLL[CATRAINING_NUM];
        S8 iFirstClkPass_3PLL[CATRAINING_NUM], iLastClkPass_3PLL[CATRAINING_NUM], iFirstCAPass_3PLL[CATRAINING_NUM], iLastCAPass_3PLL[CATRAINING_NUM];        
