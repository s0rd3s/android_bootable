...
static bool rtc_xosc_check_clock(U16 *result)
{
	...
	mdelay(10);
	print("[RTC] 0x%x, 0x%x, XMODE = %d, DCXO32K_EN = %d\n", RTC_Read(0x208), RTC_Read(0x206), RTC_Read(0x206) & 0x1, (RTC_Read(0x206) >> 1) & 0x1);
...
static bool rtc_init_after_recovery(void)
{
	...
	if (!Write_trigger())
		return false;
	print("[RTC] rtc_init_after_recovery write trigger OK\n");
	if (!Writeif_unlock())
		return false;
...
bool rtc_boot_check(void)
{
	...
	if (is_clk_buf_from_pmic()) {
		pwrap_read(0x7002, &rdata);
		rdata |= 0x8000;
		pwrap_write(0x7002, rdata);
	} else {
		clk_buf_all_on();
	}
	pmic_config_interface(FQMTR_CLK_CK_PDN_CLR, 1, FQMTR_CLK_CK_PDN_MASK, FQMTR_CLK_CK_PDN_SHIFT);
#if defined(PMIC_CHIP_MT6353)
	/* for MT6353 FQMTR CLK is separated from audio, need to config DCXO26M */
	pmic_config_interface(RG_FQMTR_32K_CLK_PDN_CLR, 1, RG_FQMTR_32K_CLK_PDN_MASK, RG_FQMTR_32K_CLK_PDN_SHIFT);
#else
	pmic_config_interface(RG_FQMTR_32K_CLK_PDN_SET, 1, RG_FQMTR_32K_CLK_PDN_MASK, RG_FQMTR_32K_CLK_PDN_SHIFT);
#endif
	...
	print("[RTC] rtc_boot_check1 powerkey1 = 0x%x, powerkey2 = 0x%x\n",
...
void rtc_save_2sec_stat(void)
{
	...
	print("[RTC] rtc_2sec_reboot_check 0x%x\n", reboot);
