...
void pmic_DetectVbatDrop (void)
{
	...
	pmic_read_interface( MT6353_STRUP_CON4, (&just_rst), PMIC_JUST_PWRKEY_RST_MASK, PMIC_JUST_PWRKEY_RST_SHIFT );
	pmic_config_interface(MT6353_STRUP_CON4, 1, PMIC_CLR_JUST_RST_MASK, PMIC_CLR_JUST_RST_SHIFT);
...
int hw_check_battery(void)
{
	...
	ret_val=pmic_config_interface( (U32)(MT6353_LDO2_TREF_CON0),
							 (U32)(1),
							 (U32)(PMIC_LDO_TREF_EN_MASK),
							 (U32)(PMIC_LDO_TREF_EN_SHIFT)
							 );

	ret_val=pmic_config_interface( (U32)(MT6353_LDO2_TREF_CON0),
							 (U32)(1),
							 (U32)(PMIC_LDO_TREF_EN_CTRL_MASK),
							 (U32)(PMIC_LDO_TREF_EN_CTRL_SHIFT)
							 );

	mt6353_upmu_set_rg_baton_en(1);
	mt6353_upmu_set_baton_tdet_en(1);
	//mdelay(100);
	val = mt6353_upmu_get_rgs_baton_undet();
...
void pl_check_bat_protect_status()
{
    ...
    while (bat_val < BATTERY_LOWVOL_THRESOLD)
    {
		...
		mdelay(50);
                 ...
                 bat_val = get_bat_sense_volt(5);
		print( "[%s]: check VBAT=%d mV  I=%d chr:%d\n", __FUNCTION__, bat_val, current, chr_volt);
    }
