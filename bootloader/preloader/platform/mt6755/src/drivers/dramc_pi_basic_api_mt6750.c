...
#ifdef IDD4_PATTERN_STRESS
const static U32 u4IDD4W_DBIoff[8] = { \
0x0f00f0ff, 0xf0fc0f03, 0x0f00f0ff, 0xf0fc0f03, 0x0f03f0fc, 0xf0ff0f00, 0xf0fc0f03, 0x0f00f0ff, \
};

const static U32 u4IDD4R_DBIoff[8] = { \
0x0f00f0ff, 0xf0fc0f03, 0x0f00f0ff, 0xf0fc0f03, 0x0f00f0ff, 0x0f03f0fc, 0xf0ff0f00, 0xf0fc0f03, \
};
...
U8 gIs_ECO_IC, gOnly_09V_1PLL_Flow_Flag;
...
void CHECK_09V_1PLL_FLOW(void)
{
    
    if (((seclib_get_devinfo_with_index(17) & 0xff) == 0x42) || ((seclib_get_devinfo_with_index(17) & 0xff) == 0x43))
    {
        gOnly_09V_1PLL_Flow_Flag = 1;
    }
    else
    {
        gOnly_09V_1PLL_Flow_Flag = 0;    
    }
...
#if __ETT__
    fab = Reg_Readl(0x10206540) & 0x40000000;
#else
    fab = seclib_get_devinfo_with_index(18) & 0x40000000;
#endif
    if (fab)
    {
        mcSHOW_DBG_MSG(("IC : FAB=1.\n")); //U
		//set clock output duty to more balence, 47%->48.2%
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0xcc), &u4value);
        u4value = (u4value & 0xffffff00) | 0xc3;
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0xcc), u4value); 
    }
    else
    {
        mcSHOW_DBG_MSG(("IC : FAB=0.\n")); //T   
    }

    if ((Reg_Readl(0x10206544)&(1<<31)) == 0)
    {
        mcSHOW_DBG_MSG(("IC : NEW CA Training Flow.\n"));
        gIs_ECO_IC = 0;
    }
    else
    {
        mcSHOW_DBG_MSG(("IC : CA Shuffle Flow.\n"));    
        gIs_ECO_IC = 1;

        //cfgreg0x65[0] (eco_en) = 1
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x194), &u4value);
        mcSET_BIT(u4value, 0);
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x194), u4value);        
    }

    //Segment code -> M_SRM_RP7  0x1020627C  bit[7:0]
    //MT6750 Turbo : 0x41
    //MT6750 Normal : 0x42
    //MT6738 : 0x43
#if __ETT__    
    mcSHOW_DBG_MSG(("IC Segment Code : 0x%x.\n", ((Reg_Readl(0x1020627c) & 0xff))));        
#else
    mcSHOW_DBG_MSG(("IC Segment Code : 0x%x.\n", ((seclib_get_devinfo_with_index(17) & 0xff))));        
#endif
    CHECK_09V_1PLL_FLOW();
...
DRAM_STATUS_T Dram_Reset(DRAMC_CTX_T *p)
{
...
void DramcDFS(DRAMC_CTX_T *p, U32 Init)
{
    ...
    if (gOnly_09V_1PLL_Flow_Flag) return;
    ...
    if (p->frequency != DUAL_FREQ_LOW)   
	{
		...
		if (!Init)
		{
			if (uiDualRank) 
			{
				uiCmpErr = DramcDmaEngine((DRAMC_CTX_T *)p, DMA_OP_READ_WRITE, 0x50000000+uiOffset, 0x80000004, 0xff000, 8, 0, 1, 0);
			}
			else
			{
				uiCmpErr = DramcDmaEngine((DRAMC_CTX_T *)p, DMA_OP_READ_WRITE, 0x50000000+uiOffset, 0x60000004, 0xff000, 8, 0, 1, 0);
			}
		}
#endif
		...
		if (!Init)
		{
			if (uiDualRank) 
			{
				uiCmpErr = DramcDmaEngine((DRAMC_CTX_T *)p, DMA_OP_READ_WRITE, 0x50000000+uiOffset, 0x80000004, 0xff000, 8, 0x0f, 1, 0);
			}
			else
			{
				uiCmpErr = DramcDmaEngine((DRAMC_CTX_T *)p, DMA_OP_READ_WRITE, 0x50000000+uiOffset, 0x60000004, 0xff000, 8, 0x0f, 1, 0);
			}
			if (uiCmpErr)
			{
...
#define FREQREG_SIZE_ECO FREQREG_SIZE+3
...
const SHUFFLE_REG LowFreq_ShuffleReg[FREQREG_SIZE_ECO] =
{
	...
	{CHA_DDRPHY_BASE + 0x188, CHA_DDRPHY_BASE + 0x1a8}, //ECO	
	{CHA_DDRPHY_BASE + 0x18c, CHA_DDRPHY_BASE + 0x1ac},	//ECO
	{CHA_DDRPHY_BASE + 0x190, CHA_DDRPHY_BASE + 0x1b0},	//ECO
	
};

static U32 FreqReg_Backup[FREQREG_SIZE_ECO+1];
...
void DramcSaveToBackup(DRAMC_CTX_T *p)
{
    ...
    {
    	for (Offset = 0; Offset < FREQREG_SIZE_ECO; Offset++)
    	{
    		FreqReg_Backup[Offset] = (*(volatile unsigned int *)(LowFreq_ShuffleReg[Offset].uiSourceAddr));
    	}
        FreqReg_Backup[FREQREG_SIZE_ECO] = (*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x0c));
    }
}
...
void DramcRestoreBackup(DRAMC_CTX_T *p)
{
    ...
    else
    {
        ...
        (*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x0c)) = FreqReg_Backup[FREQREG_SIZE_ECO]; 
    }
}
...
void DramcSaveToShuffleReg(DRAMC_CTX_T *p)
{
    ...
    else
    {
        ...
        (*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x8f0)) &= ~(0xf << 12);
		(*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x8f0)) |= (((*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x0c) >> 24)) & 0xf) << 12;    
    }
}
...
void DramcDumpFreqSetting(DRAMC_CTX_T *p)
{
    ...
    else
    {
        ...
        mcSHOW_DBG_MSG(("[Source] Addr %xh = %xh [Shuffle] Addr %xh = %xh\n", 
                    CHA_DDRPHY_BASE + 0x8f0, (*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x8f0)),
                    CHA_DDRPHY_BASE + 0x0c, (*(volatile unsigned int *)(CHA_DDRPHY_BASE + 0x0c))));
    }
}
...
#ifdef DRAMC_DRAMPHY_DCM_ENABLE
	mcSHOW_DBG_MSG(("DRAMC DRAMPHY DCM : Enable\n"));
#else
	mcSHOW_DBG_MSG(("DRAMC DRAMPHY DCM : Disable\n"));
#endif

#ifdef HW_GATING
	mcSHOW_DBG_MSG(("HW_GATING : Enable\n"));
#else
	mcSHOW_DBG_MSG(("HW_GATING : Disable\n"));
#endif

#ifdef ZQCS_ENABLE
	mcSHOW_DBG_MSG(("Runtime ZQCS : Enable\n"));
#else
	mcSHOW_DBG_MSG(("Runtime ZQCS : Disable\n"));
#endif

#ifdef EMI_OPTIMIZATION
	mcSHOW_DBG_MSG(("EMI_OPTIMIZATION : Enable\n"));
#else
	mcSHOW_DBG_MSG(("EMI_OPTIMIZATION : Disable\n"));
#endif

#ifdef CA_TRAINING_NEW_FLOW
	mcSHOW_DBG_MSG(("CA_TRAINING_NEW_FLOW : Enable\n"));
#else
	mcSHOW_DBG_MSG(("CA_TRAINING_NEW_FLOW : Disable\n"));
#endif

#ifndef NORMAL_CMD_SELPH
	mcSHOW_DBG_MSG(("CMD SELPH - 1\n"));
#endif

	ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x110), &u4value);
	if (u4value & (1<<7))
	{
		mcSHOW_DBG_MSG(("Per Bank Refresh : Enable\n"));
	}
	else
	{
		mcSHOW_DBG_MSG(("Per Bank Refresh : Disable\n"));
	}

	ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(DRAMC_REG_ACTIM1), &u4value);
	if (u4value & (1<<26))
	{
		mcSHOW_DBG_MSG(("MR4 : Disable\n"));
	}
	else
	{
		mcSHOW_DBG_MSG(("MR4 : Enable\n"));
	}
...
U32 DramcDmaEngine(DRAMC_CTX_T *p, DRAM_DMA_OP_T op, U32 src_addr, U32 dst_addr, U32 trans_len, U8 burst_len, U8 check_result, U8 ChannelNum, U8 test_pattern_type)
{
	...
	for (i = 0 ; i < (len/sizeof(unsigned int)) ; i++) {
	    *((unsigned int *)dst_buffp1+i) = 0;
        ...
        else if (test_pattern_type == 1) // 0 and 1 100% toggle pattern
        {
            memset((void *) src_buffp1, 0x55555555, len);
        }
        else if (test_pattern_type == 2) // IDD4 pattern
        {
            for(i = 0 ; i < len ; i+= (8+8)*4) {
                memcpy((void *)src_buffp1, u4IDD4W_DBIoff, 8*4);
                memcpy((void *)src_buffp1+8*4, u4IDD4R_DBIoff, 8*4);     
            }
        }
