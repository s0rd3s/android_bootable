...
#ifndef PMIC_CHIP_MT6353 /* MT6351 */
...
#else /* MT6353 */
	pmic_config_interface(MT6353_DCXO_CW15, 0x18,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	/* bit 10, 11, 12, 13 => 32K Less Mode, Buffer Mode, RTC Mode, Off Mode */
	pmic_read_interface(PMIC_XO_STATIC_AUXOUT_ADDR, &reg,
			    PMIC_XO_STATIC_AUXOUT_MASK,
			    PMIC_XO_STATIC_AUXOUT_SHIFT);
	if ((reg & 0x2000) == 0x2000) {
		print("clkbuf is from RF, DCXO_CW16=0x%x\n", reg);
		return false;
	} else {
		print("clkbuf is from PMIC, DCXO_CW16=0x%x\n", reg);
		return true;
	}
#endif
};
 ...
 * BSIMM_IMM_WDATA_1:		HW_WRITE(0x1021F008, 0x1869557)
...
void clk_buf_enable_clkbuf4(void)
{
	...
	DRV_WriteReg32(0x1021F008, 0x1869557); /* old:0x1867775 */
	DRV_WriteReg32(0x1021F004, 0x1);
	print("clk_buf_enable_clkbuf4: 0x10221100 = 0x%x\n", DRV_Reg32(0x10221100));
}
...
void clk_buf_disable_clkbuf4(void)
{
	...
	DRV_WriteReg32(0x1021F004, 0x1);
	print("clk_buf_disable_clkbuf4: 0x10221100 = 0x%x\n", DRV_Reg32(0x10221100));
}
...
void clk_buf_dump_log(void)
{
	...
	if (!is_clk_buf_from_pmic()) {
		...
		for(i=1; i<=20; i++) {
			...
			print("clkbuf: bsilogger_cw[%d]=0x%x\n", 20-i, bsilogger_cw[20-i]);
			print("clkbuf: bsilogger_ctrl[%d]=0x%x\n", 20-i, bsilogger_ctrl[20-i]);
		}

		// Sheet “3wire default” ? Set CW6[10]=0x0 and CW27[14] =0x0
		// Read CW24~CW43.
		DRV_WriteReg32( 0x1021F004, 0x0 );
		...
		for(i = 24; i < 44; i++) {
			...
			print("clkbuf: cw%d=0x%x\n", i, cw[i - 24]);
		}

		// Sheet “3wire Auxout” ? Set CW6[10]=0x1 and CW27[14] =0x1
		// Read CW24~CW43
		DRV_WriteReg32( 0x1021F004, 0x0);
		...
		for( i=24; i<44; i++) {
			...
			DRV_WriteReg32( 0x1021F620, 0x1 );
			print("clkbuf: auxout_cw%d=0x%x\n", i, auxout_cw[i - 24]);
		}

		// Sheet “3wire Auxout-dcxo” ? Set CW6[10]=0x1 and CW27[14] =0x0
		// Read CW24~CW35.
		DRV_WriteReg32( 0x1021F004, 0x0 );
		DRV_WriteReg32( 0x1021F008, 0x600784 );
		DRV_WriteReg32( 0x1021F004, 0x1 );
		...
		for( i=24; i<36; i++) {
			...
			DRV_WriteReg32( 0x1021F620, 0x1 );
			print("clkbuf: auxout_dcxo_cw%d=0x%x\n", i, auxout_dcxo_cw[i - 24]);
		}
...
void clk_buf_all_on(void)
{
	if (!is_clk_buf_from_pmic()) {
		/* RF clkbuf: VCTCXO */
		print("before clk_buf_enable_clkbuf4: 0x10221100 = 0x%x\n",
		      DRV_Reg32(0x10221100));

		clk_buf_enable_clkbuf4();
