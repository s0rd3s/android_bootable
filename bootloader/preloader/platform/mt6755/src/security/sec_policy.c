...
unsigned int get_policy_entry_idx(unsigned char *part_name) 
{
    ...
    if (i == num_of_entries) {
        print("[SEC_POLICY] reached the end, use default policy\n");
        i = 0;
    }
...
static unsigned int get_sec_state(unsigned int *sboot_state, unsigned int *lock_state)
{
    ...
    print("[SEC_POLICY] sboot_state = 0x%x\n", *sboot_state);
    if (!default_lock_state)
        print("[SEC_POLICY] lock_state = 0x%x\n", *lock_state);
    else
        print("[SEC_POLICY] lock_state(default) = 0x%x\n", *lock_state);
