                   ...
                   void *ramdisk, unsigned ramdisk_size)
{
    ...
    struct fdt_reserve_entry *pstore_rsv = NULL;
    int pstore_size, pstore_offset;
    ...
    ret = fdt_setprop(fdt, offset, "reg", mem_reg_property,
		    g_boot_arg->mblock_info.mblock_num * sizeof(dt_dram_info));
    if (ret) return FALSE;
	...
	lcm_driver_id = mt_disp_get_lcm_driver_id();
	ret = fdt_setprop(fdt, offset, "lcm_driver_id", &lcm_driver_id,
			  sizeof(lcm_driver_id));
	if (ret)
		return FALSE;

	lcm_module_id = mt_disp_get_lcm_module_id();
	ret = fdt_setprop(fdt, offset, "lcm_module_id", &lcm_module_id,
			  sizeof(lcm_module_id));
	if (ret)
		return FALSE;
#endif
...
#ifdef KERNEL_DEBUG_RAMOOPS
    pstore_offset = fdt_node_offset_by_compatible(fdt, -1, "pstore-reserve-memory");
    if (pstore_offset >= 0) {
	pstore_rsv = fdt_getprop(fdt, pstore_offset, "reg", &pstore_size);
	if (pstore_rsv < 0 || pstore_size != sizeof(*pstore_rsv))
	    dprintf(CRITICAL, "mem_reserved_pstore size %x\n", pstore_size);
	else
	    dprintf(CRITICAL, "pstore reserved memory 0x%llx, size 0x%llx\n", fdt64_to_cpu(pstore_rsv->address), fdt64_to_cpu(pstore_rsv->size));
    }
	    
    sprintf(cmdline, "%s%s%llx", cmdline, " ramoops.mem_address=0x", pstore_rsv > 0 ? fdt64_to_cpu(pstore_rsv->address) : RAMOOPS_ADDR);
    sprintf(cmdline, "%s%s%llx", cmdline, " ramoops.mem_size=0x", pstore_rsv > 0 ? fdt64_to_cpu(pstore_rsv->size) : RAMOOPS_SIZE);
    sprintf(cmdline, "%s%s%x", cmdline, " ramoops.console_size=0x", RAMOOPS_CONSOLE_SIZE);
#endif
