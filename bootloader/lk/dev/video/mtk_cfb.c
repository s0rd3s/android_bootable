			...
			while (count--) 
			{
				...
				if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
					for (tdest = pdest + row * LINE_LEN; row--; tdest -= LINE_LEN) 
					{
						...
						((unsigned int *) tdest)[0] = SHTSWAP32 ((lk_cfb_font_dtable15 [bits & 3] & cfb_eorx) ^ cfb_bgx);
					}
		...
		case CFB_565RGB_16BIT:
			while (count--) 
			{
				...
				if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
					...
					{
						unsigned char bits = *pos++;
						((unsigned int *) tdest)[3] = SHTSWAP32 ((lk_cfb_font_dtable16 [bits >> 6] & cfb_eorx) ^ cfb_bgx);
						((unsigned int *) tdest)[2] = SHTSWAP32 ((lk_cfb_font_dtable16 [bits >> 4 & 3] & cfb_eorx) ^ cfb_bgx);
						((unsigned int *) tdest)[1] = SHTSWAP32 ((lk_cfb_font_dtable16 [bits >> 2 & 3] & cfb_eorx) ^ cfb_bgx);
						((unsigned int *) tdest)[0] = SHTSWAP32 ((lk_cfb_font_dtable16 [bits & 3] & cfb_eorx) ^ cfb_bgx);
					}
					pdest = pdest + MTK_VFW * PIXEL_SIZE;
				} else {
					for (tdest = pdest;row--;tdest += LINE_LEN) 
		...
		case CFB_FMT_8BIT:
			while (count--) 
			{
				...
				if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
					...
					{
						unsigned char bits = *pos++;
						((unsigned int *) tdest)[1] = (lk_cfb_font_dtable8[bits >> 4] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[0] = (lk_cfb_font_dtable8[bits & 15] & cfb_eorx) ^ cfb_bgx;
					}
					pdest = pdest + MTK_VFW * PIXEL_SIZE;
				} else {
					for (tdest = pdest;row--;tdest += LINE_LEN) 
			...
			while (count--) 
			{
				...
				if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
					for (tdest = pdest + row * LINE_LEN; row--; tdest -= LINE_LEN) 
					{
						...
						((unsigned int *) tdest)[5] = (lk_cfb_font_dtb24[bits >> 4][0] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[4] = (lk_cfb_font_dtb24[bits >> 4][1] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[3] = (lk_cfb_font_dtb24[bits >> 4][2] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[2] = (lk_cfb_font_dtb24[bits & 15][0] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[1] = (lk_cfb_font_dtb24[bits & 15][1] & cfb_eorx) ^ cfb_bgx;
						((unsigned int *) tdest)[0] = (lk_cfb_font_dtb24[bits & 15][2] & cfb_eorx) ^ cfb_bgx;
					}
					pdest = pdest + MTK_VFW * PIXEL_SIZE;
				} else {
					for (tdest = pdest;row--;tdest += LINE_LEN) 
