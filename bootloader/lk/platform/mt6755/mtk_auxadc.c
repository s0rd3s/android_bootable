...
#define AUXADC_CG_CLR 0x1000108c
#define AUXADC_CG_SET 0x10001088
...
int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata)
{
  ...
  // mutex_lock(&mutex_get_cali_value);
   ...
   {
	    //dprintf(CRITICAL, "hwEnableClock AUXADC !!!.");
	    if(enable_clock(MT65XX_PDN_PERI_AUXADC,"AUXADC"))
	    {dprintf(CRITICAL, "hwEnableClock AUXADC failed.");}
        
   }
	*/

   *(volatile unsigned short *)AUXADC_CG_CLR = 0x400;
   ...
   //dprintf(CRITICAL, "[adc_api: imm mode raw data => channel[%d] = %d\n",dwChannel, channel[dwChannel]);
   //dprintf(CRITICAL, "[adc_api]: imm mode => channel[%d] = %d.%d\n", dwChannel, (channel[dwChannel] * 250 / 4096 / 100), ((channel[dwChannel] * 250 / 4096) % 100));
   ...
   */
   //*(volatile unsigned short *)AUXADC_CG_SET = 0x400;
   return 0;
...
int IMM_GetOneChannelValue_Cali(int Channel, int*voltage)
{
     ...
     if(ret)
     {
	   ...
	   if(ret)
	   {
	        dprintf(CRITICAL, "[adc_api]:IMM_GetOneChannelValue_Cali  get raw value error %d \n",ret);
		  return -1;
	   }
     }
     ...
     *voltage = (int)((long long)rawvalue*1500000 / (long long)4096);		     
      //dprintf(CRITICAL, "[adc_api]:IMM_GetOneChannelValue_Cali  voltage= %d uv \n",*voltage);
...
static int IMM_auxadc_get_evrage_data(int times, int Channel)
{
	...
	while (i--)
	{
		...
		ret += ret_temp;
		dprintf(CRITICAL, "[auxadc_get_data(channel%d)]: ret_temp=%d, ret = ret_value.\n",Channel,ret_temp,ret_value);        
	}
...
int auxadc_test(void ) 
{
   ...
   //int Voltiage_cali =0;
      ...
      for (i = 0; i < 16; i++) 
      {
        //dprintf(CRITICAL, "[adc_driver]: i=%d\n",i);
		...
		{
		       dprintf(CRITICAL, "[adc_lk]: channel[%d]raw =%d\n",i,rawdata);
		       //dprintf(CRITICAL, "[adc_lk]: channel[%d]=%d.%.02d \n",i,data[0],data[1]);
		...
		if(res < 0)
		{ 
			   dprintf(CRITICAL, "[adc_driver]: get cali voltage error\n");
			   break;
			   
		}
		else
		{
		       dprintf(CRITICAL, "[adc_driver]: channel[%d] cali_voltage =%d\n",i,Voltiage_cali);
