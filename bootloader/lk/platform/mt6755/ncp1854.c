...
void ncp1854_hw_init()
{
    ...
    ncp1854_set_wdto_dis(0x0);  //enable wdt timer
...
void ncp1854_charging_enable(kal_uint32 bEnable)
{
    ...
    if (g_chr_type_num == STANDARD_CHARGER)
    {
        if(g_std_ac_large_current_en==1)
        {
            ...
            #ifdef NCP1854_PWR_PATH
            ncp1854_set_ichg_high(1);	//ICHG
            ncp1854_set_ichg(0x5);	//Fast Charging Current Limit at 2.5A
            #else
            if(bat_val < PRECC_BATVOL)
                ncp1854_set_ichg(0x1);  //Pre-Charging Current Limit at 500mA
            else
                ncp1854_set_ichg(0xE);	//Fast Charging Current Limit at 1.9A
            #endif
        }
        else
        {
            temp_CC_value = 1500;
            ncp1854_set_iinlim_ta(0xA); //IN current limit at 1500
            if(bat_val < PRECC_BATVOL)
                ncp1854_set_ichg(0x1);  //Pre-Charging Current Limit at 500mA
            else
      ...
      || g_chr_type_num == NONSTANDARD_CHARGER)
    {
        ...
        #ifdef NCP1854_PWR_PATH
        ncp1854_set_ichg(0x4);  //Charging Current Limit at 800mA
        #else
        ncp1854_set_ichg(0x1);  //Charging Current Limit at 500mA
        #endif
    }
    else
    {
        ...
        ncp1854_set_iinlim(0x1); //IN current limit at 500mA
        ncp1854_set_ichg(0x1);  //Charging Current Limit at 500mA
        dprintf(CRITICAL, "[BATTERY:ncp1854] unknown charger type");
    }
    ...
    ncp1854_set_wdto_dis(0x0); //reset wdt timer
    dprintf(CRITICAL, "[BATTERY:ncp1854] ncp1854_set_ac_current(), CC value(%dmA) \r\n", temp_CC_value);
    dprintf(CRITICAL, "[BATTERY:ncp1854] charger enable/disable %d !\r\n", bEnable);
}
