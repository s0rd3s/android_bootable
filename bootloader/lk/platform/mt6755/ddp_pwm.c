...
{
	unsigned int regosc;
	regosc = PWM_REG_GET(OSC_ULPOSC_ADDR);
	PWM_MSG("ULPOSC config : 0x%08x", regosc);

	/* OSC EN = 1 */
	regosc = regosc | 0x1;
	PWM_REG_SET(OSC_ULPOSC_ADDR, regosc);
	regosc = PWM_REG_GET(OSC_ULPOSC_ADDR);
...
int disp_pwm_clk_update(void)
{
	unsigned int regsrc = PWM_REG_GET(MUX_UPDATE_ADDR);/* set clock source update bit */
