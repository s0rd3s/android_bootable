...
static int mboot_android_check_bootimg_hdr(part_dev_t *dev, char *part_name, boot_img_hdr *boot_hdr)
{
	...
	unsigned int bootimg_hdr_vfy_time = 0;
#endif
...
#ifdef MTK_SECURITY_SW_SUPPORT
	bootimg_hdr_vfy_time = get_timer(0);
    ...
    if (img_auth_required) {
        ret = (int)sec_img_auth_init("boot", "boot");   
        if (ret)
            return ret;
    } 
    dprintf(CRITICAL, "[SBC] boot cert vfy(%d ms)\n", (unsigned int)get_timer(bootimg_hdr_vfy_time));
#endif
...
#ifdef MTK_SECURITY_SW_SUPPORT
	bootimg_hdr_vfy_time = get_timer(0);
    policy_entry_idx = get_policy_entry_idx("recovery");
    img_auth_required = get_vfy_policy(policy_entry_idx);
    /* verify cert chain of boot img */
    if (img_auth_required) {
        ret = (int)sec_img_auth_init("recovery", "boot");   
        if (ret)
            return ret;
    } 
    dprintf(CRITICAL, "[SBC] recovery cert vfy(%d ms)\n", (unsigned int)get_timer(bootimg_hdr_vfy_time));
#endif
...
int mboot_android_load_bootimg(char *part_name, unsigned long addr)
{
    ...
    unsigned int policy_entry_idx = 0;
    unsigned int img_auth_required = 0;
	unsigned int bootimg_vfy_time = 0;
#endif
#ifdef MTK_EMMC_SUPPORT
	unsigned long long start_addr;
#else
...
#ifdef MTK_SECURITY_SW_SUPPORT
	bootimg_vfy_time = get_timer(0);
    policy_entry_idx = get_policy_entry_idx("boot");
    img_auth_required = get_vfy_policy(policy_entry_idx);
    if (img_auth_required) {
        ret = (int)sec_img_auth(addr, g_bimg_sz - g_pg_sz);
        if (ret)
            len = -EIO;
    }
    dprintf(CRITICAL, "[SBC] boot vfy(%d ms)\n", (unsigned int)get_timer(bootimg_vfy_time));
#endif
...
int mboot_android_load_recoveryimg(char *part_name, unsigned long addr)
{
    ...
    unsigned int policy_entry_idx = 0;
    unsigned int img_auth_required = 0;
	unsigned int recoveryimg_vfy_time = 0;
#endif
#ifdef MTK_EMMC_SUPPORT
	unsigned long long start_addr;
#else
...
#ifdef MTK_SECURITY_SW_SUPPORT
	recoveryimg_vfy_time = get_timer(0);
    policy_entry_idx = get_policy_entry_idx("recovery");
    img_auth_required = get_vfy_policy(policy_entry_idx);
    if (img_auth_required) {
        ret = (int)sec_img_auth(addr, g_rcimg_sz - g_pg_sz);
        if (ret)
            len = -EIO;
    }
    dprintf(CRITICAL, "[SBC] recovery vfy(%d ms)\n", (unsigned int)get_timer(recoveryimg_vfy_time));
#endif
...
int mboot_android_load_factoryimg(char *part_name, unsigned long addr)
{
    ...
    dprintf(CRITICAL, "len = %d, addr = 0x%lx\n", len, addr);
...
int mboot_recovery_load_raw_part(char *part_name, unsigned long *addr, unsigned int size)
{
    ...
    dprintf(CRITICAL, "[%s] Load '%s' partition to %08p (%d bytes in %ld ms)\n", MODULE_NAME, part->name, addr, size, get_timer(begin));
...
int mboot_recovery_load_misc(unsigned long *misc_addr, unsigned int size)
{
	...
	printf("[mboot_recovery_load_misc]: misc_addr is %p\n", misc_addr);
