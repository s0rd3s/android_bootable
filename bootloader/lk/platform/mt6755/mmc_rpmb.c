...
extern int32_t rpmb_hmac(uint32_t offset, uint32_t size);
...
int mmc_rpmb_read_data(u32 blkAddr, u32 *buf, u32 bufLen)
{
	...
	for (i = 0; i < blkLen; i++) {
		...
		ret = mmc_rpmb_start_req(card, &rpmb_req);
		if (ret) {
			printf("%s, mmc_rpmb_start_req failed!! (%x)\n", __func__, ret);
			return ret;
		}
